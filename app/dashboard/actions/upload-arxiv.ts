...
async function fetchArxivMeta(
    ...
    const apiTitle = titleMatch ? titleMatch[1].trim().replace(/\s+/g, " ") : "";
...
/** Extracts a bare arXiv ID from a URL or ID string. Returns null if invalid. */
    ...
    export async function uploadArxiv(arxivInput: string, userTitle: string): Promise<UploadArxivResult> {
    ...
    try {
        ...
        if (exists.length > 0) {
        return { status: "error", message: "This paper has already been uploaded." };
        }
        ...
        return {
            status: "error",
            message: "arXiv did not return a PDF. Check the ID and try again.",
        };
        }
        ...
        for (let offset = 0; offset < chunkRows.length; offset += embeddingBatchSize) {
        ...
        return { status: "error", message: "Failed to fetch or process the arXiv paper." };
    }
}